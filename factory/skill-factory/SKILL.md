...
## הערה על `site_gate`
התבנית כותבת אוטומטית `site_gate: "open"` לכל סקיל חדש, כך שהוא מוצג בגלוי
באתר כברירת מחדל. אין צורך לשאול על זה בשלב 2. חסימת סקיל מאחורי טופס מייל
(`site_gate: "gated"`) היא החלטה שיווקית נפרדת, שמתבצעת רק אם אלעד מבקש
במפורש, כשינוי שורה אחת בקובץ ה-`SKILL.md` של אותו סקיל בלבד.
