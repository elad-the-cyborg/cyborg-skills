...
//   category="קופי ותוכן" topic="הוקים וזוויות" title_he="מחולל הוקים הסייבורג" \
//   level=beginner one_liner="..." trigger="..." tags="..." step1_title="..." example_prompt="..."
const ROOT = new URL('../', import.meta.url).pathname
const args = Object.fromEntries(process.argv.slice(2).map(a => {
  const i = a.indexOf('='); return [a.slice(0, i), a.slice(i + 1)]
}))
for (const req of ['name', 'category_slug', 'category', 'topic', 'title_he', 'level']) {
  if (!args[req]) { console.error(`missing arg: ${req}`); process.exit(1) }
}
const vars = { tags: '', one_liner: '', trigger: '', step1_title: '', example_prompt: '', ...args }
